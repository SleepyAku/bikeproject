...
if( $order_id == "")
{
   ...
   $insertcolumns = "customer_id,order_status,order_date_required_date,shipped_date,store_id,staff_id";
...
if( $product_id != "")
{
    ...
    // standard php code for getting data
...
//This will get a table of all the products on this order_id 
$db = new DatabaseClass("production.products",false,$global_serverName,$global_connectionInfo);
...
$tsql = "SELECT sum(list_price * quantity) as subtotal from sales.order_items where order_id = ".$order_id;
$db->Select($tsql, "order_id");    


$subtotal = strval($db->getFieldByColumnName("subtotal"));
