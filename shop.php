...
{
    $tsql = "select p.product_id, p.product_name, p.list_price, p.image from production.products p where (select count(*) from production.stocks s where s.store_id = ".$_GET['store_id'] ." and s.quantity > 0 and s.product_id = p.product_id ) > 0 order by p.product_name";
}
else
{
